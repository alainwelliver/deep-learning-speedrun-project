"""
airbench94_deterministic_with_backoff.py

MODIFICATION: Deterministic Translation Augmentation + Backoff Schedule
========================================================================
Extends the deterministic translation idea with a curriculum-style schedule:
use deterministic translation for the early epochs, then "back off" to
flip-only augmentation to reduce augmentation overhead late in training.

HYPOTHESIS:
- Deterministic translation early in training improves data coverage and
  convergence behavior.
- Turning off translation (backoff) after a certain epoch reduces per-epoch
  compute without significantly hurting final accuracy.
"""
...
hyp = {
    ...
    'aug': {
        ...
        # NEW: stop using translation after this epoch index
        # (data loader's self.epoch is 0,1,2,... per outer epoch)
...
def batch_crop(images, crop_size):
    ...
    if r <= 2:
        ...
        for s in range(-r, r+1):
            mask = (shifts[:, 0] == s)
            images_tmp[mask] = images[mask, :,
                                      r+s:r+s+crop_size,
                                      :]
        for s in range(-r, r+1):
            mask = (shifts[:, 1] == s)
            images_out[mask] = images_tmp[mask, :,
                                          :,
                                          r+s:r+s+crop_size]
    return images_out
...
class CifarLoader:
    ...
    MODIFIED: Supports deterministic translation + backoff schedule.
    """

    def __init__(self, path, train=True, batch_size=500,
                 aug=None, drop_last=None, shuffle=None, gpu=0):
        ...
        for k in self.aug.keys():
            assert k in ['flip', 'translate',
                         'deterministic_translate',
                         'translate_backoff_epoch'], f'Unrecognized key: {k}'
        ...
        if self.base_positions is not None:
            ...
            for pos in range(self.num_positions):
                idx = torch.nonzero(self.base_positions == pos,
    ...
    def __iter__(self):
        ...
        # Precompute normalized / flipped / padded once on first epoch
        if self.epoch == 0:
        ...
        pad = self.aug.get('translate', 0)
        backoff_epoch = self.aug.get('translate_backoff_epoch', None)
        deterministic = self.aug.get('deterministic_translate', False)

        # Decide whether to use translation this epoch
        use_translate = (
            pad > 0
            and (backoff_epoch is None or self.epoch < backoff_epoch)
        )
        ...
        if use_translate:
            if deterministic:
                ...
                assert self.position_groups is not None, (
                    "position_groups must be precomputed when "
                    "deterministic_translate is True"
                )
        ...
        # Alternating flip (same as original airbench idea)
        if self.aug.get('flip', False):
            if self.epoch % 2 == 1:
...
def init_whitening_conv(layer, train_set, eps=5e-4):
    patches = get_patches(train_set,
                          patch_shape=layer.weight.data.shape[2:])
    eigenvalues, eigenvectors = get_whitening_parameters(patches)
    eigenvectors_scaled = eigenvectors / torch.sqrt(eigenvalues + eps)
    layer.weight.data[:] = torch.cat((eigenvectors_scaled,
                                      -eigenvectors_scaled))
...
def main(run):
    ...
    train_loader = CifarLoader('cifar10', train=True,
                               batch_size=batch_size, aug=hyp['aug'])

    # Fast-fail if we somehow ended up on CPU instead of CUDA
    assert train_loader.images.device.type == "cuda", (
        f"Train images device is {train_loader.images.device}, "
        "expected CUDA. Check Runpod / GPU setup."
    )
    ...
    param_configs = [
        dict(params=norm_biases,
             lr=lr_biases,
             weight_decay=wd/lr_biases),
        dict(params=other_params,
             lr=lr,
             weight_decay=wd/lr)
    ]
    ...
    lr_schedule = triangle(total_train_steps,
                           start=0.2, end=0.07, peak=0.23)
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lambda i: lr_schedule[i]
    )

    alpha_schedule = 0.95**5 * (
        torch.arange(total_train_steps+1) / total_train_steps
    )**3
    ...
    for epoch in range(ceil(epochs)):
        ...
        train_acc = (
            outputs.detach().argmax(1) == labels
        ).float().mean().item()
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("MODIFICATION: Deterministic Translation + Backoff")
    print("=" * 60)
    print()
