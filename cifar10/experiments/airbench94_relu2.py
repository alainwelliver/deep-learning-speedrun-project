...
class SquaredReLU(nn.Module):
    ...
    Reference: https://arxiv.org/abs/2109.08668v2 (Primer paper)
    Used in NanoGPT speedrun with ~4% improvement.
    """
    def forward(self, x):
        return F.relu(x).square()

def get_activation(activation_type='gelu'):
    """Get activation module by type.

    Args:
        activation_type: 'gelu' or 'relu2'

    Returns:
        nn.Module: Activation function module

    Raises:
        ValueError: If activation_type not supported
    """
    if activation_type == 'gelu':
        ...
        raise ValueError(f"Unsupported activation: {activation_type}. Use 'gelu' or 'relu2'.")

class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum, activation='gelu'):
        super().__init__()
