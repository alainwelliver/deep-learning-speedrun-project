...
def depthwarm_config(step: int, total_steps: int, n_layers: int):
    """
    Depth warmup schedule.

    Returns (max_depth, p_stoch) where:
      - max_depth: layers with index < max_depth are always active.
      - p_stoch: probability of skipping layers with index >= max_depth during training.
    """
    if total_steps <= 0:
        return n_layers, 0.0
    ...
    if frac < 1.0 / 3.0:
        # First third: use about half the layers, heavy stochastic skipping on the rest.
        max_depth = max(1, n_layers // 2)
        p_stoch = 0.5
    elif frac < 2.0 / 3.0:
        # Middle third: ~75% depth, lighter stochastic skipping.
        max_depth = max(1, int(0.75 * n_layers))
        p_stoch = 0.25
    else:
        # Final third: full depth, no skipping.
        max_depth = n_layers
        p_stoch = 0.0

    return max_depth, p_stoch


class Block(nn.Module):

    def __init__(self, config, layer_idx: int, n_layers: int):
        super().__init__()
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # DepthWarm bookkeeping: these are set by the training loop.
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([
                Block(config, layer_idx=i, n_layers=config.n_layer)
...
raw_model.total_steps = args.num_iterations
raw_model.global_step = 0
