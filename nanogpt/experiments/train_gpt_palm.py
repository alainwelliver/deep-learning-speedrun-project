# NOTE: Modified version of train_gpt.py implementing PaLM Parallel Block
# Original record: https://github.com/KellerJordan/modded-nanogpt
# ====================================================================================================
import os
import sys
# We read the code of this file ASAP for logging, even though we modified it
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # PaLM-style Parallel Attention & MLP with Shared Norm
        # This replaces the sequential: x = x + attn(ln(x)); x = x + mlp(ln(x))
        
        # 1. Shared Norm: Calculate once
        x_norm = F.rms_norm(x, (x.size(-1),))
        
        # 2. Parallel Compute (torch.compile will fuse these ops where possible)
        attn_out = self.attn(x_norm)
        mlp_out = self.mlp(x_norm)
        
        # 3. Sum with scaling
        # Scale by 1/sqrt(2) approx 0.7071 to maintain variance consistency 
        # with the sequential addition of two residual branches.
        x = x + 0.7071 * (attn_out + mlp_out)
        return x
